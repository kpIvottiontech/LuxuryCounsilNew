...
class HomeController extends GetxController {
  ...
  await  POST_API(articlelistRequest, Urls.SUBSCRIBE_USER_ARTICLE_LIST, context)
     ...
     // print("total_page response ----> ${map["data"]['total_page']}");
      ...
      if (map["status"] != true) {
        ...
        if (map['app_user_status'] == 0) {
         ...
         // print(" nonsubscribeResponse ----> ${nonsubscribeResponse.nonSubscribeModel!.article!.first.articleName}");
          ...
          nosubsvideoList
              .addAll((nonsubscribeResponse.data!.video ?? []));
          nosubsarticleList
              .addAll(nonsubscribeResponse.data!.article ?? []);
          total_page.value = nonsubscribeResponse.totalPage?.toInt() ?? 0;
          print('total_page ----> ${total_page.value}');
